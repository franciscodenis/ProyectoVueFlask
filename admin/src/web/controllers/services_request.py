...
def service_requests_index():
    ...
    pagination = service_requests.list_service_request(page, service_type, start_date, end_date, state, user_id)
    servicios = pagination.items

    return render_template("service_requests/index.html", servicios=servicios, pagination=pagination)
...
def service_request_show(request_id):
    ...
    return render_template('service_requests/show.html', service_request=service_request)
...
def service_request_update(request_id):
    ...
    if form.validate_on_submit():
        ...
        if result:
            flash('La solicitud de servicio se ha actualizado correctamente.', 'success')
            return redirect(url_for('service_requests.service_requests_index'))
        else:
    ...
    return render_template('service_requests/update.html', form=form, service_request=service_request)
